                    ...
                    <section>
                        <header>
                            ...
                            <p class="mt-1 text-sm text-gray-600">
                                {{ __('Add a bank account for invoice payments.') }}
                            </p>
                        </header>
                        ...
                        <form method="POST" action="{{ route('bank-accounts.store') }}" class="mt-6 space-y-6">
                            ...
                            <div>
                                ...
                                <select id="currency" name="currency" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm uppercase" required>
                                    ...
                                    @foreach(config('currencies.allowed', []) as $currencyCode)
                                        <option value="{{ $currencyCode }}" @selected(old('currency') === $currencyCode)>
                                            {{ $currencyCode }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="iban" name="iban" type="text" class="mt-1 block w-full uppercase" :value="old('iban')" required maxlength="34" />
                            ...
                            <div>
                                ...
                                <x-text-input id="nickname" name="nickname" type="text" class="mt-1 block w-full" :value="old('nickname')" maxlength="255" />
                                <x-input-error class="mt-2" :messages="$errors->get('nickname')" />
                                <p class="mt-1 text-xs text-gray-500">Optional friendly name (e.g., "Main Account", "EUR Account")</p>
                            </div>

                            <div class="flex items-center">
                                <input id="is_default" name="is_default" type="checkbox" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(old('is_default'))>
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <x-primary-button>{{ __('Create Bank Account') }}</x-primary-button>
                            </div>
                        </form>
                    </section>
                </div>
